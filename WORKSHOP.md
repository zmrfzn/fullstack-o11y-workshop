...
In this challenge, we will manually install and configure the Infrastructure agent for **Debian 13 (trixie)**.
...
### Step 3 – Enable the repository (Debian 13 trixie)

Add the repo to the APT sources list:

```bash
echo "deb [signed-by=/etc/apt/trusted.gpg.d/newrelic-infra.gpg] https://download.newrelic.com/infrastructure_agent/linux/apt/ trixie main" | sudo tee /etc/apt/sources.list.d/newrelic-infra.list
```
