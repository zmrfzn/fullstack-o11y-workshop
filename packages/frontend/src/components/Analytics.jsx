...
const Analytics = () => {
  ...
  */
  ...
  const processData = (tutorials, categories) => {
    ...
    if (categoryFilter) {
      filteredTutorials = filteredTutorials.filter(t => t.category === categoryFilter.id);
    }

    // Attempt to get the first tutorial's metadata for display
    const firstTutorial = filteredTutorials[0];
    const firstCategory = firstTutorial.category.trim();
